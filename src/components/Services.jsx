...
export default function Services({ lang = "es" }) {
  ...
  const services = [
    {
      ...
      items: isEnglish
        ? ["Slow laptop or phone", "Possible virus", "Won’t turn on"]
        : ["Laptop o celular lento", "Posible virus", "No enciende"],
      url: "/device-repair-ajijic",
    },
    {
      title: isEnglish ? "📶 WiFi & Network Issues" : "📶 Problemas de WiFi",
      items: isEnglish
        ? ["Slow WiFi", "Keeps disconnecting", "No coverage"]
        : ["WiFi lento", "Se desconecta", "Sin señal"],
      url: "/wifi-problems-ajijic",
    },
    {
      title: isEnglish ? "🛡️ Security & Cameras" : "🛡️ Cámaras y seguridad",
      items: isEnglish
        ? ["Install cameras", "Smart doorbell", "Monitor home"]
        : ["Instalar cámaras", "Timbre inteligente", "Monitoreo del hogar"],
      url: "/security-cameras-ajijic",
    },
    {
      title: isEnglish ? "🏠 Tech Support" : "🏠 Soporte técnico",
      items: isEnglish
        ? ["Computer help", "Account access", "Not sure how to fix"]
        : [
            "Ayuda con computadora",
            "Acceso a cuentas",
  ...
  return (
    ...
    >
      ...
      <p className="text-center text-gray-600 dark:text-gray-300 mb-4 md:mb-5 max-w-md mx-auto">
        {isEnglish
          ? "Pick your issue. We’ll fix it."
          : "Elige tu problema. Nosotros lo solucionamos."}
      </p>
        ...
        {services.map((service) => (
          ...
          >
            <div className="h-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-2 md:p-3 shadow-md flex flex-col justify-between hover:scale-[1.02] hover:shadow-2xl hover:border-primary dark:hover:bg-gray-800 transition-all duration-300 cursor-pointer active:scale-95 group">
              ...
              <ul className="text-xs md:text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
